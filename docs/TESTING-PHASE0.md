...
**Phase 2.x (life):** organism, chaos, **CAMP camper**, Computer neuron, and water-column behaviour are covered by Catch2 unit tests in `evo-lab-tests` — filters `[water]`, `[chaos]`, `[nom]` (legacy tag for camper tests), and `[camp][computer]`. Design and invariants are documented in [DESIGN-NOTES.md](DESIGN-NOTES.md) §2.5, §2.7, §2.8, §3.1 (water bands), §4.4.1.
